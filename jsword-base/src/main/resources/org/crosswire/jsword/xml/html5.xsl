...
<xsl:stylesheet
    ...
    <xsl:template match="note">
        ...
        <xsl:variable name="first">
            <xsl:choose>
                <xsl:when test="$prev-position >= 0 and name($siblings[$prev-position]) = 'note'">
                    <xsl:value-of select="''"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <a href="#{generate-id(.)}" class="nm note sup{$first}">
    ...
    <xsl:template name="prefix-match">
        ...
        <xsl:if test="string-length($str)">
            ...
            <xsl:variable name="match">
                <xsl:choose>
                    <xsl:when test="starts-with($str, $prefix)">
    ...
    <xsl:template name="notate">
        ...
        <xsl:if test="string-length($strongs)">
            <span class="w">
                <span>
                    <xsl:value-of select="substring-before(concat($pos, ' '), ' ')"/>
                </span>

                <xsl:variable name="lemma-work" select="substring-after(substring-before($lemma, ':'), '.')"/>
                <xsl:variable name="lemma-next"
                              select="substring-after(substring-before(concat($lemma, ' '), ' '), ':')"/>
                <span class="lemma" onclick='app.uri("sword://{$lemma-work}/{$lemma-next}"); return false;'>
                    <xsl:value-of select="$lemma-next"/>
                </span>

                <xsl:variable name="strong-work" select="substring-before($strongs, ':')"/>
                <xsl:variable name="strong-next"
                              select="substring-after(substring-before(concat($strongs, ' '), ' '), ':')"/>
                <span class="strongs" onclick='app.uri("sword://{$strong-work}/{$strong-next}"); return false;'>
                    <xsl:value-of select="format-number(substring($strong-next,2),'#')"/>
                </span>

                <xsl:variable name="morph-work" select="substring-before($morph, ':')"/>
                <xsl:variable name="morph-next"
                              select="substring-after(substring-before(concat($morph, ' '), ' '), ':')"/>
                <span class="morph" onclick='app.uri("sword://{$morph-work}/{$morph-next}"); return false;'>
                    <xsl:value-of select="$morph-next"/>
                </span>
            </span>
            <xsl:call-template name="notate">
    ...
    <xsl:template name="lemma">
        ...
        <xsl:if test="string-length($lemma)">
            ...
            <a class="lemma" href="sword://{$work}/{$next-lemma}">
                <xsl:value-of select="$next-lemma"/>
            </a>
    ...
    <xsl:template name="strongs">
        ...
        <xsl:if test="string-length($lemma)">
            ...
            <xsl:variable name="next-lemma" select="substring-before(concat(substring-after($lemma, ':'), ' '), ' ')"/>
            <a class="strongs" href="sword://{$work}/{$next-lemma}">
                <xsl:value-of select="format-number(substring($next-lemma,2),'#')"/>
            </a>
    ...
    <xsl:template name="morph">
        ...
        <xsl:if test="string-length($morph)">
            ...
            <a class="morph" href="sword://{$work}/{$next-morph}">
                <xsl:value-of select="$next-morph"/>
            </a>
    ...
    <xsl:template match="milestone[@type = 'x-p']">
        <xsl:text> </xsl:text><xsl:value-of select="@marker"/><xsl:text> </xsl:text>
    </xsl:template>
    <xsl:template match="milestone[@type = 'x-p']" mode="jesus">
        <xsl:text> </xsl:text><xsl:value-of select="@marker"/><xsl:text> </xsl:text>
    </xsl:template>
    ...
    <xsl:template name="generateFontStyle">
        ...
        <xsl:variable name="aFont">
            <xsl:choose>
                <xsl:when test="substring-before($fontspec, $fontSeparator) = ''"><xsl:value-of select="$fontspec"/>,0,16
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$fontspec"/>
                </xsl:otherwise>
        ...
        <xsl:variable name="fontsize"
                      select="substring-after(substring-after($aFont, $fontSeparator), $fontSeparator)"/>
        <xsl:variable name="styling"
                      select="substring-before(substring-after($aFont, $fontSeparator), $fontSeparator)"/>
