...
<xsl:stylesheet
        ...
        <xsl:if test="$NoVNum = 'false'">
            ...
            <xsl:choose>
                ...
                <xsl:when test="$TinyVNum = 'false' and $Notes = 'true'">
                    <a name="{@osisID}">(<xsl:value-of select="$versenum"/>)
                    </a>
                    <xsl:text> </xsl:text>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template name="lemma">
        ...
        <xsl:choose>
            <xsl:when test="$separator = ''">
                <sub class="strongs">
                    <a href="{$protocol}{$orig-lemma}">S
                        <xsl:number level="any" from="/osis//verse" format="1"/>
                        <xsl:number value="$sub" format="a"/>
                    </a>
                </sub>
            </xsl:when>
            <xsl:otherwise>
                <sub class="strongs">
                    <a href="{$protocol}{substring-before($orig-lemma, $separator)}">S
                        <xsl:number level="single" from="/osis//verse" format="1"/>
                        <xsl:number value="$sub" format="a"/>
                    </a>
                    ,
                </sub>
    ...
    <xsl:template name="morph">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <sub class="morph">
                    <a href="{$protocol}{substring-before($orig-morph, $separator)}">M
                        <xsl:number level="single" from="/osis//verse" format="1"/>
                        <xsl:number value="$sub" format="a"/>
                    </a>
                    ,
