...
<xsl:stylesheet
    ...
    <xsl:template match="verse[not(@eID)]">
        ...
        <xsl:if test="milestone[@type = 'x-p'] or q[@who = 'Jesus']/milestone[@type = 'x-p']">
            <br/>
            <br/>
        </xsl:if>
        ...
        <xsl:if test="$NoVNum = 'false'">
            ...
            <xsl:choose>
                ...
                <xsl:when test="$TinyVNum = 'false' and $Notes = 'true'">
                    <a name="{@osisID}">
                        <font class="verse">(<xsl:value-of select="$versenum"/>)
                        </font>
                    </a>
                    <xsl:text>&#160;</xsl:text>
                </xsl:when>
    ...
    <xsl:template name="lemma">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$separator = ''">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <sub class="strongs">
                            <a class="strongs" href="{$protocol}{$orig-lemma}">
                                <xsl:value-of select="format-number(substring($orig-lemma,2),'#')"/>
                            </a>
                        </sub>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="$protocol = $lex.def.protocol">
                        <sub class="lemma"><xsl:value-of select="substring-before($orig-lemma, $separator)"/>,
                        </sub>
                    </xsl:when>
                    <xsl:otherwise>
                        ...
                        <sub class="strongs">
                            <a class="strongs" href="{$protocol}{substring-before($orig-lemma, $separator)}">
                                <xsl:value-of
                                        select="format-number(substring(substring-before($orig-lemma, $separator),2),'#')"/>
                            </a>
                            ,
                        </sub>
                    </xsl:otherwise>
    ...
    <xsl:template name="morph">
        ...
        <xsl:choose>
            <xsl:when test="$separator = ''">
                ...
                <sub class="morph">
                    <a href="{$protocol}{$orig-morph}">
                        <xsl:value-of select="$orig-morph"/>
                    </a>
                </sub>
            </xsl:when>
            <xsl:otherwise>
                ...
                <sub class="morph">
                    <a href="{$protocol}{substring-before($orig-morph, $separator)}">
                        <xsl:value-of select="substring-before($orig-morph, $separator)"/>
                    </a>
                    ,
                </sub>
                <xsl:call-template name="morph">
    ...
    <xsl:template match="hi">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$style = 'small-caps'">
                <font class="small-caps">
                    <xsl:apply-templates mode="small-caps"/>
                </font>
            </xsl:when>
            <xsl:when test="$style = 'spaced-letters'">
                <font class="spaced-letters">
                    <xsl:call-template name="spaced-letters">
