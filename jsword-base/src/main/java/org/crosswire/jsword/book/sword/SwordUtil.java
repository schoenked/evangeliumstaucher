...
public final class SwordUtil {
     ...
     *             on error
     */
    static byte[] readRAF(RandomAccessFile raf, long offset, int theSize) throws IOException {
        raf.seek(offset);
     ...
     *             on error
     */
    static byte[] readNextRAF(RandomAccessFile raf, int theSize) throws IOException {
        ...
        if (size < 0) {
            log.error("Nothing to read at offset = {} returning empty because negative size={}", offset, size);
            return new byte[0];
        }

        if (offset >= rafSize) {
            log.error("Attempt to read beyond end. offset={} size={} but raf.length={}", offset, size, rafSize);
            return new byte[0];
        }

        if (offset + size > raf.length()) {
            log.error("Need to reduce size to avoid EOFException. offset={} size={} but raf.length={}", offset, size, rafSize);
     ...
     *             on error
     */
    static void writeRAF(RandomAccessFile raf, long offset, byte[] data) throws IOException {
        raf.seek(offset);
        writeNextRAF(raf, data);
    }

    private static void writeNextRAF(RandomAccessFile raf, byte[] data) throws IOException {
        if (data == null) {
            return;
        }
        raf.write(data);
     ...
     *             on error
     */
    private static byte[] readUntilRAF(RandomAccessFile raf, int offset, byte stopByte) throws IOException {
        raf.seek(offset);
     ...
     *             on error
     */
    static byte[] readUntilRAF(RandomAccessFile raf, byte stopByte) throws IOException {
     ...
     *            the offset into the array
     */
    static void encodeLittleEndian32(int val, byte[] data, int offset) {
     ...
     * @return The decoded data
     */
    static int decodeLittleEndian16(byte[] data, int offset) {
        // Convert from a byte to an int, but prevent sign extension.
        // So -16 becomes 240
        int byte1 = data[offset] & 0xFF;
        int byte2 = (data[1 + offset] & 0xFF) << 8;
     ...
     *            the offset into the array
     */
    static void encodeLittleEndian16(int val, byte[] data, int offset) {
        data[offset] = (byte) (val & 0xFF);
        data[1 + offset] = (byte) ((val >> 8) & 0xFF);
    }
     ...
     */
    static int findByte(byte[] data, byte sought) {
     ...
     * @return The index of the found position or -1 if not found
     */
    static int findByte(byte[] data, int offset, byte sought) {
    ...
    private static void clean1252(String key, byte[] data, int offset, int length) {
        ...
        for (int i = offset; i < end; i++) {
            ...
            if ((c >= 0x00 && c < 0x20 && c != 0x09 && c != 0x0A && c != 0x0D) || (c == 0x81 || c == 0x8D || c == 0x8F || c == 0x90 || c == 0x9D)) {
                data[i] = 0x20;
                log.error("{} has bad character 0x{} at position {} in input.", key, Integer.toString(c, 16), i);
            }
        }
    }
