...
public final class PassageKeyFactory {
     ...
     */
    private static final PassageKeyFactory keyf = new PassageKeyFactory();
     ...
     */
    private static int binarySize(int max) {
        // 1 byte (2^8)
        if (max < 256) {
            return 1;
        }
     ...
     */
    static int toBinary(byte[] buffer, int index, int number, int max) {
     ...
     */
    static int fromBinary(byte[] buffer, int[] index, int max) {
