...
public final class ReadEverything {
    ...
    private static void testReadMultiple(Book book, Key set) {
        ...
        if (count > 0) {
            log.info("Tested: book={} entries={} time={}s errors={} ({}ms per entry)", book.getInitials(), entries, time, count, 1000 * time / entries);
        } else {
            log.info("Tested: book={} entries={} time={}s ({}ms per entry)", book.getInitials(), entries, time, 1000 * time / entries);
        }
    }
    ...
    private static void testReadSingle(Book book, Key key, int entry) {
        Throwable oops = null;
        try {
            log.debug("reading: {}/{}:{}", book.getInitials(), entry, key.getName());
        ...
        if (oops != null) {
            ++count;
            if (count < 5) {
                log.error("Unexpected error reading: {}, {}, {}, {}", book.getInitials(), entry, key.getOsisID(), key.getClass().getName(), oops);
            }
        }
    }
}
