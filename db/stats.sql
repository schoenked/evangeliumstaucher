...
SELECT
    TO_CHAR(date_series.date, 'DD.MM.YYYY') AS day,
    ...
    COALESCE((
                 SELECT COUNT(DISTINCT gs.player_id)
                 FROM game_session_entity gs
                 WHERE gs.id IN (
                     SELECT game_session_id
                     FROM user_question_entity
                     WHERE answered_at::date = date_series.date
                 )
             ), 0) AS anzahl_spieler,
                 ...
                 SELECT STRING_AGG(DISTINCT (
                     ...
                     COALESCE((
                                  ...
                                  WHERE gs2.player_id = p.id
                                    AND uqe2.answered_at::date = date_series.date
                              ), 0) || ')'
                     ), ', ')
                 FROM game_session_entity gs
